...
* diagnostics module compares user's recipe list to known working combinations
* prompt to set -T or -O args (but not when generating application templates)
* extensive conditional configuration options with a prefs{} hash (the 'quiz')
...
* conversion of ERB files to Haml or Slim by 'copy_from_repo' method
...
* add 'rvm' option to 'extras' recipe to greate a project-specific gemset and .rvmrc file
