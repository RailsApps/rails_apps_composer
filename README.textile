...
The "Rails Composer":http://railsapps.github.com/rails-composer/ tool is built with the rails_apps_composer gem. The Rails Composer tool creates the "Rails Example Applications":http://railsapps.github.com/.

Any issues? Please create a "GitHub issue":http://github.com/RailsApps/rails_apps_composer/issues.

h4. What's New in Version 2.0

Rails_apps_composer 2.0 is significantly different from earlier versions:

* improved "quiz" that sets preferences
* recipes are based on functionality (authentication, testing) instead of gems (Devise, RSpec)
* more downloaded template files rather than manipulation of files with 'gsub'
* README for any starter app with details from the recipes

Most important, many unsupported or outdated recipes have been removed. If you need a missing recipe, please update it and test it with version 2.0, then submit a pull request. I'm delighted to add new recipes. You'll find documentation for developers of recipes in the wiki.

h4. Alternatives

See a list of "Rails Application Template Projects":http://railsapps.github.com/rails-application-templates.html.

The rails_apps_composer gem is derived from "Michael Bleigh's RailsWizard gem":https://github.com/intridea/rails_wizard (see "differences":#rails_apps_composer-differences).
...
You'll be prompted for recipes:

<pre>
$ rails_apps_composer new myapp
</pre>

Or provide a list of recipes:
...
<pre>
$ rails_apps_composer new myapp --defaults=~/.rac
</pre>
...
h4. Generate a Template from Defaults

<pre>
$ rails_apps_composer template ~/Desktop/template.txt --defaults=~/.rac
</pre>
...
Generate an app from defaults:

<pre>
$ rails_apps_composer new myapp --defaults=~/.rac
</pre>

Or generate a template from defaults:
